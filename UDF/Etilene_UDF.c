...
{
    real source = -q_wall / (DELTAH * Volume);
    /* if the heat flux is inverted, the mass flow won't be negative */ 
    if (source < 0.0)
    {
        source = 0;
    }
...
DEFINE_SOURCE(ETILENE_mass_source, cell, cell_thread, dS, eqn)
{
    ...
    c_face_loop(cell, cell_thread, n)
    {
        ...
        if (THREAD_ID(face_thread) == PARAFFIN_WALL_ID)
        {
            ...
            /* Mass term */
            source = compute_mdot_source_iso(q_wall, ETILENE_DELTAH, Volume);
            break;
        }
    }
...
DEFINE_SOURCE(ETILENE_momentum_source_y, cell, cell_thread, dS, eqn)
{
    ...
    c_face_loop(cell, cell_thread, n)
    {
        ...
        if (THREAD_ID(face_thread) == PARAFFIN_WALL_ID)
        {
            ...
            /* Mass flow rate */
            mdot = compute_mdot_source_iso(q_wall, ETILENE_DELTAH, Volume);
