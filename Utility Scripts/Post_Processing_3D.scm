...
(define (export-line-data line-name variable iter x y z)
...
(define tan-vel "tangential-velocity")
...
; Extracting values of target variables on surfaces in the axial direction
(export-line-data line1 var1 iter 1 0 0)
(export-line-data line1 var2 iter 1 0 0)
(export-line-data line1 var3 iter 1 0 0)
(export-line-data line1 var4 iter 1 0 0)
(export-line-data line1 var5 iter 1 0 0)
(export-line-data line1 var6 iter 1 0 0)
(export-line-data line2 var5 iter 1 0 0)
(export-line-data line2 var6 iter 1 0 0)
(export-line-data line3 var5 iter 1 0 0)
(export-line-data line3 var6 iter 1 0 0)
; Extracting mean values of target variables on target planes
...
;; Loop to export data on lines and planes
(do ((i 0 (+ i 1)))
    ((>= i num-lines) 'done)
  ; Recover vertical line name
  (define num (* i 2))
  (define line-name (format #f "line-~a" num))

  ; Extract axial velocity on the line
  (export-line-data line-name ax-vel iter x1 y1 z1)
  ; Extract swirl velocity on the line
  (export-line-data line-name tan-vel iter x1 y1 z1)
  ; Extract temperature on the line
  (export-line-data line-name temp iter x1 y1 z1)

  ; Recover plane name
  (define plane-name (format #f "plane-~a" i))
  
  ; Recover middle line name
  (define num (+ num 1))
  (define line-name (format #f "line-~a" num))

  ; Extract axial velocity on the line
  (export-line-data line-name ax-vel iter x2 y2 z2)
  ; Extract swirl velocity on the line
  (export-line-data line-name tan-vel iter x2 y2 z2)
  ; Extract temperature on the line
  (export-line-data line-name temp iter x2 y2 z2)

  ; Compute angular momentum flux through the plane
  (compute-quantity plane-name ang-mom iter)
  ; Compute axial momentum flux through the plane
  (compute-quantity plane-name ax-vel iter)
